...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/09/23 13:02:49 by akrifa            #+#    #+#             */
/*   Updated: 2023/09/23 17:00:59 by akrifa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */


void	**fill_case(int **tab, int i, int j, int size)
{
	if (j == 9)// cas reussite
		return (tab);
	if ( == )  // tout les patterns on echoues
		return (NULL);
	if (j == 11)
		return (fill_case(tab, i + 1, 1, size)); // cas on descend
	if (j == 0)
		return (fill_case(tab, i - 1, 9, size));//cas on remonte
